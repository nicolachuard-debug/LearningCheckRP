...
def _get_client():
    ...
    return _clientdef _build_config(temperature: Optional[float], max_output_tokens: Optional[int]):
...
def _guess_mime_type(filename: Optional[str], fallback: str = DEFAULT_MIME_TYPE) -> str:
    ...
    return guessed or fallbackdef generate_text(
    prompt: str,
    model_name: str = DEFAULT_MODEL,
    temperature: Optional[float] = None,
    max_output_tokens: Optional[int] = None,
) -> str:
    """Genera testo a partire da un prompt testuale."""
...
def generate_from_image(
    ...
    """Genera testo a partire da un prompt e un'immagine."""
    if not prompt or not prompt.strip():
        raise ValueError("Il prompt non può essere vuoto.")
    if not image_bytes:
